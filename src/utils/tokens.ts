...
export interface TokenInfo {
  ...
  readonly tokenType: 'GENERAL' | 'SABER_LP' | 'cTOKEN';
...
export const mainnetTokens: TokenInfos = {
  xeUSDC: {
    symbol: 'xeUSDC',
    name: 'xeUSD Coin',
  ...
  SOL_mSOL: {
    ...
    decimals: 9,
    logoURI:
      'https://raw.githubusercontent.com/solana-labs/token-list/main/assets/mainnet/5ijRoAHVgd5T5CNtK5KDRUBZ7Bffb69nktMj5n6ks6m4/logo.png',
  },
  cSOL_cmSOL: {
    chainId: 101,
    ...
    decimals: 9,
    logoURI:
      'https://raw.githubusercontent.com/solana-labs/token-list/main/assets/mainnet/SSoxcNry3qU1wN5Vj6zMJKTSqJPyPLks5KNfCJCmUPa/icon.png',
  },
  cUSDC_cUSDT: {
    chainId: 101,
...
export const getDecimalsFromMint = (mint) => {
...
export const getTokenFromSymbol = (symbol: string) => {
  return Object.entries(TOKENS).find((f) => f[1].symbol === symbol)?.[1];
};

export const getTokenFromMintAddress = (mintAddress: string) => {
  return Object.entries(TOKENS).find(
    (f) => f[1].mintAddress === mintAddress
  )?.[1];
};
